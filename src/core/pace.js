...
MR.Pace = (function () {
  ...
  const EFFORT_CFG = {
     ...
     *
     * The owner: "energy needs to drain quicker as the game progresses. just
     * how you get tired faster through a race."
     *
     * So the burn is not flat. It scales with how far you have come:
     *
     *     rate(u) = (1 / DRAIN_SECONDS) * (1 + FATIGUE * u)
     *
     * where u is the fraction of the marathon completed. At FATIGUE = 1 the
     * last mile burns at exactly twice the rate of the first.
     *
     * DRAIN_SECONDS WAS RE-BASED RATHER THAN LEFT ALONE, and that is the
     * whole reason the tuning below this line survives. The integral of the
     * expression above over a whole race is (1 + FATIGUE/2) times the flat
     * rate, so a fatigue curve bolted onto the old constant would have
     * silently drained half a tank more than the number every dial was set
     * against. 5000 became 7500 = 5000 * 1.5, which leaves the TOTAL burn of
     * a race exactly what it was and changes only its SHAPE.
     *
     * What that buys, and it is the point: the early miles are cheap and the
     * late ones are dear, so a tank that looks comfortable at halfway is not.
     * The player who banks energy early is the one who still has legs at mile
     * 24 -- which is the actual sport, and it lands the danger squarely in
  ...
  function create(elev) {
    ...
    s.update = function (dt) {
      ...
      if (EFFORT > 0) {
        // Fatigue: the burn climbs with the distance already run, so the last
        // mile costs twice the first. Read off miles rather than the clock so
        // a slow runner is not punished twice for being slow -- they are
        // already paying in seconds, and charging them extra energy for the
        // same stretch of road would compound a bad run into an unrunnable
        // one. See ENERGY_FATIGUE.
        const done = s.miles / K.MARATHON_MILES;
        const fatigue = 1 + EFFORT_CFG.ENERGY_FATIGUE * (done > 1 ? 1 : done);
        s.energy -= (dRace / EFFORT_CFG.ENERGY_DRAIN_SECONDS) * fatigue;
