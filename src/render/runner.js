 ...
 *   THIS CHARACTER HAD NO BROW LINE, AND THE OWNER MOVED THE HAT TO GIVE HIM
 *     ONE. The eye's sclera reaches HY + 0.035 and the cap band's underside sat
 *     at HY + 0.023, so the brim was 0.012 INTO the top of each eye and there
 *     was nowhere for a brow to be. The first pass wrote that up as "what
 *     wearing a cap pulled down means" -- a limitation dressed as an intention
 *     -- and the owner's own quality bar refutes it: both reference characters
 *     have heavy dark brows on bare forehead BELOW the brim, in the same hat,
 *     because their brim sits higher. The cost was named (the hat moves, and
 *     the hat is his identity), the owner was asked, and they chose to pay it.
 *     Band centre HY + 0.060 -> HY + 0.102, carrying the crown shell's lower
 *     edge and the peak with it.
 *
 *     What 0.042 buys is 0.0302 of bare forehead, which is enough for a brow to
 *     SIT and not enough for one to TRAVEL -- so the brow was moved in FRONT of
 *     the whole eye stack in z instead, where it can cut across the top of the
 *     iris. That one decision took the available tilt from about four degrees
 *     to nine, and it is the difference between a brow that is present and a
 *     brow that furrows.
 *   TWO CORRECT ANIMATION TERMS CAN SUBTRACT. The brow and the eye's own
 *     aperture were both defensible and, measured together, read LESS than the
 *     brow alone -- 8585 changed pixels against 11668 -- because a symmetric
 *     aperture pulls the eye's top edge down and away exactly as the brow comes
 *     down to meet it. The fix was not smaller gains but a different EDGE: the
 *     aperture now closes from the BOTTOM, the pivot translating by half the
 *     height it loses so the top edge is pinned. The pair then measures 13713,
 *     which is a swing of 6062 pixels for one line of geometry reasoning and no
 *     change in gains. Whenever two terms drive the same feature, measure them
 *     TOGETHER as well as apart; "both are good" is not a measurement.
...
MR.Runner = (function () {
  ...
  const SHOE_DK = 0x3a4270;  // heel counter and ankle collar -- the sheet's navy accents
  ...
  // ---- HANDS, NOT MITTS (docs/miles.md, the anatomy pass) ----------------
  // The blue was a measured decision -- gloves that flash against the road --
  // and the owner's verdict on the portrait outranks it: "He still does not
  // look like a human in those photos." Blue balls on the ends of the arms
  // were the loudest single toy-read on the figure. The flash duty moves to
  // the trim wristbands, which sit at the same height and were already the
  // brighter mark; the hand itself goes to a skin a step deeper than the
  // face, which is what the back of a real hand does.
  // Deepened a second step in round three of the anatomy pass: at the first
  // skin value the hand VANISHED against the thigh behind it in the portrait
  // -- same tone, same light -- and the arm and leg fused into one column,
  // which read as more of a balloon-man than the blue ever did. The back of
  // a hand is tanned and shadowed against a thigh in life too.
  const MITT = 0xe09c6a;
  ...
  const MITT_DK = 0xc4854f;   // finger shadow, on skin now -- see MITT above
  ...
  const IRIS = 0x6a3d1c;   // warm brown, per the character sheet
  ...
  function eyePieces() {
    ...
    for (const s of [-1, 1]) {
      const x = s * 0.100;
      out.push(
        { g: new THREE.CircleGeometry(0.073, R_TORSO), c: SCLERA, x: x, y: y, z: 0.2610, sy: 0.82 },
        // 0.054 against the sheet's giant glossy iris -- the eye is the
        // sheet's single loudest feature and the iris is most of the eye.
        { g: new THREE.CircleGeometry(0.054, R_TORSO), c: IRIS, x: x, y: y - 0.006, z: 0.2630 },
        { g: new THREE.CircleGeometry(0.027, R_NUB), c: P.ink, x: x, y: y - 0.006, z: 0.2650 },
        ...
        // one as a hole. The second, smaller dot low and opposite is the
        // sheet's two-catchlight gloss -- it is what makes the iris a sphere
        // of glass rather than a disc of paint.
        { g: new THREE.CircleGeometry(0.016, R_NUB), c: CATCH, x: x - s * 0.019, y: y + 0.021, z: 0.2670 },
        { g: new THREE.CircleGeometry(0.008, R_NUB), c: CATCH, x: x + s * 0.016, y: y - 0.026, z: 0.2670 }
      );
    }
    return out;
  }

  // How long half a brow is, in the brow mesh's own space. Hoisted because the
  // rest pose, the per-vertex tilt weight and the clearance arithmetic in the
  // expression block all have to agree on it.
  const BROW_HALF = 0.0465;

  // The sclera's own half-height: CircleGeometry(0.062) squashed to 0.82. The
  // expression block needs it to hold the eye's TOP edge still while the lower
  // lid closes, and deriving it twice is how two numbers drift apart.
  // Follows the sclera's radius: the anatomy pass grew the eye 18% (the
  // features were lost on the head's acreage -- every reference character
  // wears eyes twice this figure's old proportion) and a blink constant
  // that lags its own eye pins the wrong edge.
  const EYE_SEMI = 0.073 * 0.82;

  /**
   * The brows, and they exist because the owner moved the hat for them.
   *
   * There was no forehead on this character until the brim went up 0.042 (see
   * the band). What that bought is 0.0302 of bare skin between the eye's top at
   * HY + 0.035 and the band's underside at HY + 0.065, and 0.0302 is enough for
   * a brow to sit and not enough for one to travel.
   *
   * THE FIX IS THE Z, NOT THE Y. The brow is at z 0.2690, which is IN FRONT OF
   * THE WHOLE EYE -- sclera 0.2610, iris 0.2630, pupil 0.2650, catchlight
   * 0.2670. Behind them, a descending brow would have disappeared under the
   * eye and the drive would have been capped at the 0.0034 of skin under the
   * rest pose. In front of them it CUTS the top of the eye, which is what a
   * furrow physically is, and the travel is bounded by the PUPIL instead: at
   * full effort and a full wince the inner end reaches HY + 0.0243, still
   * 0.0163 clear of the pupil's top at HY + 0.008. That is 9.2 degrees of tilt
   * on top of a 4.0 degree rest angle, where staying behind the eye would have
   * bought about four in total.
   *
   * THE CLEARANCE TARGET THIS PASS NAMED WAS NOT MET AND SHOULD NOT HAVE BEEN.
   * The 0.042 lift was derived to give "one ink width of clearance over the
   * eye", and what came out is 0.0034 at rest and a deliberate OVERLAP under
   * load. Both halves of the target turned out to be wrong once the z was
   * settled: an ink width is the clearance two SHADED surfaces need so their
   * outline shells do not interpenetrate, and this brow is welded into a flat
   * unlit mesh that has no ink shell at all -- there is nothing to grow teeth.
   * And a brow that never touches the eye cannot furrow. The number that
   * replaced it is the pupil clearance above.
   *
   * That is also why the brow is the primary expression channel and the eye's
   * own aperture is now secondary -- see the expression block, where both were
   * measured before either was believed.
   *
   * P.runnerHair, which is his own hair, so no colour enters the character
   * here. Heavy: 0.093 long and 0.015 deep, against an eye 0.124 wide. The
   * reference's brows are the heaviest mark on either of its faces and thin
   * ones would be the version of this that is technically present.
   */
  function browPieces() {
    const out = [];
    for (const s of [-1, 1]) {
      out.push({
        // sy 0.25 -> 0.40 and a step darker than the hair: the sheet's brows
        // are its heaviest facial mark, thick blocks rather than lines.
        g: new THREE.CircleGeometry(0.030, R_TORSO), c: 0x452a12,
        x: s * 0.100, y: 0, z: 0.2690, sx: 1.60, sy: 0.40, rz: s * 0.07,
      });
    }
    return out;
  }
  ...
  /**
   * Per-vertex weights for the brow's tilt: -1 at the inner end of each brow,
   * +1 at the outer end.
   *
   * The tilt has to MIRROR -- both inner ends drop together -- and a single
   * mesh cannot be rotated two ways at once. Two meshes would have done it and
   * cost a second draw call; rewriting the position buffer costs none, and it
   * is the same technique the two hems use, for the same reason.
   */
  function browRig(mesh) {
    const pos = mesh.geometry.attributes.position;
    const rest = new Float32Array(pos.array);
    const u = new Float32Array(pos.count);
    for (let i = 0; i < pos.count; i++) {
      const t = (Math.abs(rest[i * 3]) - 0.100) / BROW_HALF;
      u[i] = t < -1 ? -1 : t > 1 ? 1 : t;
    }
    return { pos, rest, u };
  }
  ...
  function mouthPieces() {
    // Rebuilt for the new character: the rest face is CONFIDENT, not
    // labouring, so the rest mouth is a small closed smile -- a thin arc
    // bowed downward with its corners turned up by two dots of the same
    // tube radius, which is the reference's own drawing. The expression
    // machinery keeps working unchanged because it drives the PIVOT's
    // scales: joy widens the arc (scale.x), effort deepens the bow into an
    // open oval (scale.y), a wince narrows and deepens it -- one piece,
    // four readable states.
    const ARC = 1.35;
    const cx = 0.052 * Math.cos(-Math.PI / 2 + ARC / 2);
    const cy = 0.052 * Math.sin(-Math.PI / 2 + ARC / 2);
    return [
      { g: new THREE.TorusGeometry(0.052, 0.0105, 8, 18, ARC), c: MOUTH,
        rz: -Math.PI / 2 - ARC / 2, z: 0.040 },
      { g: new THREE.SphereGeometry(0.0105, 10, 8), c: MOUTH, x: -cx, y: cy + 0.004, z: 0.040 },
      { g: new THREE.SphereGeometry(0.0105, 10, 8), c: MOUTH, x: cx, y: cy + 0.004, z: 0.040 },
    ];
  }
   ...
   */
  /**
   * ================= LATHE: THE ORGANIC-FORM FOUNDATION ==================
   *
   * The owner's overhaul brief, condensed: "cartoon proportions + intentional
   * character design", "the face must have a clear front-facing surface
   * rather than being features attached to a ball", "avoid visible seams
   * where primitive shapes meet". A figure assembled from spheres cannot
   * comply, however carefully the spheres are placed -- the seams ARE the
   * construction.
   *
   * This is the way out, and it is one function: a profile -- a hand-drawn
   * list of (radius, height) pairs -- revolved into a single continuous
   * solid. A head stops being a sphere and becomes a drawn line from crown
   * to chin; a torso becomes shoulder-chest-waist-hip in one surface; a limb
   * carries its elbow swell IN the curve, so there is no bead because there
   * is no second shape. Smooth normals come from the revolution itself.
   *
   * The profile is resampled through a Catmull-Rom pass so the hand-authored
   * points can be sparse -- eight points draw a skull -- and the surface
   * still bends, not creases. Ends are capped by pinching the profile to
   * r=0, which keeps every solid closed on all sides per the standing rule.
   *
   * `opts.sx/sz` squash the revolution off-round (a head is deeper than it
   * is wide, a torso wider than deep); `opts.tint` darkens per-vertex colour
   * toward the bottom of the form -- the cheap, bake-time ambient occlusion
   * the shading phase of the brief asks for, in the same vertex-colour
   * channel weld() already carries.
   */
  function lathe(profile, c, o) {
    o = o || {};
    const pts = [];
    const P = profile;
    const N = 4;                       // resample density between authored points
    ...
    pts.push(new THREE.Vector2(Math.max(0.0001, P[P.length - 1][0]), P[P.length - 1][1]));
    // LatheGeometry's winding assumes the profile runs BOTTOM-TO-TOP; every
    // profile in this file is authored crown-down because that is how a form
    // is described, and fed in that order the whole revolution comes out
    // INSIDE-OUT -- front faces culled, so the surface you see is the inner
    // back wall. The silhouette is identical, which is what let four of these
    // ship before the fault surfaced as the shorts drawing OVER the vest (a
    // nested part beats a culled front at the depth test every time). Found
    // by hiding parts on the live page, one at a time. Normalised here so
    // authoring order stays free.
    if (pts.length > 1 && pts[0].y > pts[pts.length - 1].y) pts.reverse();
    const g = new THREE.LatheGeometry(pts, o.seg || 26);
  ...
  /**
   * Turn the bottom band of an already-welded garment into cloth.
   *
   * `part` is what multi() returned, so its fill and its ink shell SHARE one
   * geometry object -- which is the property that makes this cheap and is also
   * the property that makes it correct: rewriting the buffer moves the outline
   * with the hem instead of leaving a black line behind on the body. The bib
   * learned that the hard way and its note records it.
   *
   * Weight ramps from 0 at `yFlat` to 1 at `yEdge` on a smoothstep. Smooth
   * rather than linear because the ramp's own kink is visible: a linear weight
   * puts a crease across the garment at yFlat that travels with the flutter,
   * and at three tones on a toon ramp a moving crease reads as a seam.
   *
   * The rest pose is kept and the deformation is written from it every frame.
   * Absolute, never incremental -- see the bib.
   */
  function hemCloth(part, yFlat, yEdge) {
    const pos = part.userData.fill.geometry.attributes.position;
    const rest = new Float32Array(pos.array);
    const w = new Float32Array(pos.count);
    const th = new Float32Array(pos.count);
    const r0 = new Float32Array(pos.count);
    for (let i = 0; i < pos.count; i++) {
      const x = rest[i * 3], y = rest[i * 3 + 1], z = rest[i * 3 + 2];
      const t = (yFlat - y) / (yFlat - yEdge);
      w[i] = t <= 0 ? 0 : t >= 1 ? 1 : t * t * (3 - 2 * t);
      th[i] = Math.atan2(x, z);
      r0[i] = Math.sqrt(x * x + z * z);
    }
    return { pos, rest, w, th, r0 };
  }

  /**
   * Drive one hem. A travelling wave around the garment, one-sided.
   *
   * ONE-SIDED is the bib's bug fix applied before it can happen again: a
   * signed sine drives half of every cycle INWARD, and a hem has a body
   * behind it. The vest hem sits 0.006 outside the shorts and the short cuff
   * 0.008 outside the thigh, so an inward swing of any usable amplitude puts
   * the garment through what it is worn over -- which does not read as cloth,
   * it reads as a hole. Biased into [0, 1], the hem alternates between lifted
   * and flat, which is what a hem does.
   *
   * Radial, about the garment's own axis, because both of these are bodies of
   * revolution: scaling x and z lifts the hem away from the body all the way
   * round, where moving it along one axis would shear the front into the back.
   * The vertical term is UP only -- cloth lifting off a waist rises, and a hem
   * that dropped would eat the margin tools/footroom.js asserts.
   */
  function hemFlutter(h, amp, t, lobes, up) {
    const a = h.pos.array;
    for (let i = 0; i < h.pos.count; i++) {
      const w = h.w[i];
      if (w <= 0) continue;
      const lift = (0.5 + 0.5 * Math.sin(t + h.th[i] * lobes)) * amp * w;
      const k = h.r0[i] > 1e-5 ? 1 + lift / h.r0[i] : 1;
      a[i * 3] = h.rest[i * 3] * k;
      a[i * 3 + 2] = h.rest[i * 3 + 2] * k;
      a[i * 3 + 1] = h.rest[i * 3 + 1] + lift * up;
    }
  ...
  // Radii re-derived when the vest became a lathe (its own note). The card
  // and the vest share the same 0.78 z-squash, so clearance is pure radius:
  // the vest's front reaches 0.2607 at the card's top edge and 0.2366 at its
  // bottom, which puts the card 0.015 proud at the top -- the old 0.268 was
  // 0.007 INSIDE the new chest and the top rows z-fought as a white tear --
  // and lets the bottom bow off the waist pull-in the way pinned paper does.
  const BIB_R0 = 0.276;        // radius at the top edge...
  const BIB_R1 = 0.264;        // ...and at the bottom, bridging the waist
  const BIB_H = 0.146;
  ...
  function create() {
    ...
    const pelvis = multi([
      // 0.222/0.216, from 0.228/0.220: the shorts are worn UNDER the vest and
      // the vest lathe's waist is 0.234, so the old 0.228 left 0.006 between
      // two 24-segment surfaces -- inside facet-interpenetration range -- and
      // the navy surfaced through the red in the portrait. 0.012 of clearance
      // is real; the shorts still own everything below the hem.
      { g: new THREE.CylinderGeometry(0.222, 0.216, 0.25, R_TORSO), c: P.runnerShort, y: -0.020, sz: 0.78 },
      // The sheet's shorts carry a bright side stripe down each hip.
      { g: new THREE.BoxGeometry(0.010, 0.150, 0.052), c: TRIM, x: -0.2205, y: -0.038 },
      { g: new THREE.BoxGeometry(0.010, 0.150, 0.052), c: TRIM, x: 0.2205, y: -0.038 },
    ...
    const trunk = multi([
      ...
      // ---- PHASE 5 OF THE OVERHAUL: THE TORSO IS ONE DRAWN SURFACE ------
      // Trapezius slope into shoulder width, chest, a real waist pull-in,
      // then the vest hem flaring back over the hips -- the brief's own
      // list, as one lathe. The old cylinder read as the rounded block the
      // brief named. Widest point stays 0.262, exactly the cylinder's own,
      // so the trunk's contribution to every half-width contract is pinned.
      // Two of these numbers were wrong the first time and both faults were
      // photographed before they were reasoned about:
      //   THE WAIST WAS 0.220, and the pelvis cylinder under it is 0.228 --
      //     so the "waist pull-in" pulled the vest INSIDE the shorts and the
      //     navy punched through the red across the whole midriff. A vest is
      //     worn OVER the shorts; its narrowest point must still clear the
      //     widest thing beneath it. 0.234 keeps a real pull-in from the
      //     0.262 shoulder while staying 0.006 outside the pelvis.
      //   THE SPLINE OVERSHOOTS. Catmull-Rom through trapezius 0.192 ->
      //     shoulder 0.262 arrives at the shoulder still climbing, and the
      //     curve peaked at 0.266 -- past the pinned width, and within a
      //     float of the bib's card. The extra point at [0.256, 0.070] hands
      //     the shoulder a flat tangent so the profile crests ON the stated
      //     number instead of after it.
      // sz 0.78 rather than 0.80: the bib's slab and the pelvis both squash
      // z by 0.78, and sharing the ellipse is what lets the bib's clearance
      // be checked as pure radius at every angle instead of at three.
      { g: lathe([
          [0.100, 0.138],   // collar opening, seating the new short neck
          [0.192, 0.106],   // trapezius slope
          [0.256, 0.070],   // pre-shoulder, pinning the tangent (see above)
          [0.262, 0.044],   // shoulder line -- the old cylinder's own width
          [0.253, -0.010],  // upper chest
          [0.244, -0.058],  // chest
          [0.234, -0.098],  // the waist -- outside the pelvis, see above
          [0.238, -0.158],  // hem flaring over the hips
          [0.148, -0.186],  // under-hem tuck
          [0.001, -0.196],  // closed underneath, all sides built
        ], null, { seg: 24 }), c: P.runnerVest, y: -0.030, sz: 0.78, ry: Math.PI },
      ...
      { g: new THREE.SphereGeometry(0.130, R_LIMB, 16), c: P.runnerVest, x: -0.240, y: 0.008, sy: 0.74, sz: 0.82 },
      { g: new THREE.SphereGeometry(0.130, R_LIMB, 16), c: P.runnerVest, x: 0.240, y: 0.008, sy: 0.74, sz: 0.82 },
      ...
      { g: new THREE.CylinderGeometry(0.152, 0.146, 0.030, R_TORSO), c: JACKET, y: 0.118, sz: 0.86 },
      // ---- the chest mark ----------------------------------------------
      // The sheet's tee carries a small white running-figure on the right
      // pec. Five welded pieces, 0.06 tall, standing 0.004-0.010 off the
      // lathe's curvature -- a mark, not a decal, so it survives every
      // angle. His right is -x (the panel faces +z with the face).
      { g: new THREE.SphereGeometry(0.012, 10, 8), c: 0xfdfcf6, x: -0.076, y: 0.064, z: 0.202 },
      { g: new THREE.BoxGeometry(0.011, 0.032, 0.005), c: 0xfdfcf6, x: -0.080, y: 0.038, z: 0.202, rz: 0.30 },
      { g: new THREE.BoxGeometry(0.008, 0.026, 0.004), c: 0xfdfcf6, x: -0.064, y: 0.046, z: 0.202, rz: -1.15 },
      { g: new THREE.BoxGeometry(0.008, 0.028, 0.004), c: 0xfdfcf6, x: -0.092, y: 0.016, z: 0.202, rz: 0.80 },
      { g: new THREE.BoxGeometry(0.008, 0.024, 0.004), c: 0xfdfcf6, x: -0.066, y: 0.014, z: 0.202, rz: -0.45 },
      ...
      // Resized against the LATHE rather than the cylinder it was tuned on:
      // at its own height the new surface is r 0.236, and the old 0.240 ring
      // sat 0.004 off it -- under one ink width, which is the teeth threshold
      // this file keeps re-learning -- and rendered as a red band floating
      // over the shorts. 0.250 stands a full 0.014 proud, and taller by
      // 0.004 so the band still owns pixels over the flare below it.
      { g: new THREE.CylinderGeometry(0.250, 0.246, 0.034, R_TORSO), c: JACKET, y: -0.166, sz: 0.82 },
    ]);
    chest.add(trunk);

    // ---- the two hems that were still welded on -----------------------------
    //
    // Stage 2 established the rule this file now runs on: BODY PARTS GET LAG
    // FILTERS, CLOTH GETS SPRINGS. It then gave springs to the two largest
    // pieces of cloth the back view sees -- the hood and the race bib -- and
    // stopped, which left the two pieces the eye actually tracks the WAIST on
    // still nailed to the skeleton: the vest's hem and the short's cuff.
    //
    // Those two are worth more than their size suggests, because of where they
    // are rather than how big they are. The vest hem is the largest value edge
    // on the upper half of the figure (the waistband was cut for cutting it by
    // 41%, see the pelvis), and the short cuff is the edge the thighs come out
    // of. A hem that never moves is the single most reliable tell that a
    // garment is painted on rather than worn.
    //
    // THEY COST NOTHING TO DRAW, and that is why they are done this way rather
    // than by giving each hem its own mesh on its own pivot. A separate part
    // is two draw calls -- fill and ink -- and draw calls are the binding
    // constraint on this project at ~266 against ~400. Instead the welded
    // geometry's own position buffer is rewritten each frame, exactly as the
    // bib's is, so a hem that moves costs the same as a hem that does not.
    //
    // Absolute, never incremental, for the reason the bib's note gives: an
    // incremental deformation accumulates float error over a two-hour race and
    // the garment slowly inflates off the body.
    //
    // HE HAS NO SLEEVES. The owner's list asks for follow-through on the vest,
    // the sleeves and the shorts; the base-layer sleeve was removed two passes
    // ago for reading as a hole in the vest and has stayed out on a measured
    // argument, so there is nothing there to move. Said here rather than left
    // as a silent omission.
    const vestHem = hemCloth(trunk, -0.120, -0.181);
    const shortHem = hemCloth(pelvis, -0.085, -0.148);
    ...
    // THE HOOD IS GONE, and it was found by elimination rather than by taste.
    // The character-overhaul portrait showed a navy panel covering the entire
    // chest, and every plausible suspect was argued over -- the bib, the
    // shorts, a z-fight -- until the parts were hidden one at a time on the
    // live page: the panel was the hood. A torus with 0.25 of reach lying
    // FLAT around the neck presents its whole underside to a portrait camera
    // sitting below it, and 0.50 of navy annulus projected over the chest is
    // a bib-shaped hole in the vest. The old vest cylinder happened to mask
    // most of the sight line; the lathe's trapezius slope opened it.
    //
    // It does not come back smaller, because the garment itself was a relic:
    // the runner wore a hooded top two costume passes ago, and a race vest
    // has no hood. The pivot stays -- parts.hood is compared by name in
    // tools/pose-diff.js and the spring code poses it -- it just poses air.
    ...
    // The two hems' own clocks. Two of them, at different rates, because one
    // clock driving both would tie the waist and the thigh line together and
    // the pair would read as one hoop rather than as two garments.
    let vestT = 0, shortT = 0;
    ...
    let faceT = 0;
    // ---- the animation-quality pass's own state ---------------------------
    // airPrev watches for the touchdown edge; landK is the landing impulse it
    // throws (decaying, eased before use). leanC and leanN are two lagged
    // copies of the bank input -- a lag filter and an underdamped spring --
    // so a lane change reads hips-first, chest-then, head-last-with-a-settle
    ...
    const headMesh = multi([
      ...
      // Thicker than it was (0.104/0.122 -> 0.118/0.134): the portrait showed
      // a dowel between collar and chin, and a neck that fills toward the jaw
      // reads as anatomy where a thin cylinder reads as an axle.
      // Phase 4 of the overhaul: short and tapered, seated up under the jaw
      // so head-jaw-neck-shoulder reads as one construction. The old SCM
      // ridges and fillet ring below were sized for the tall dowel and now
      // do their work against this shorter column.
      { g: new THREE.CylinderGeometry(0.094, 0.126, 0.21, R_LIMB), c: P.runnerSkin, y: HY - 0.252 },
      ...
      { g: new THREE.TorusGeometry(0.100, 0.022, 8, 20, Math.PI * 1.11), c: P.runnerSkin,
        y: HY - 0.236, rx: -Math.PI / 2, rz: Math.PI * 0.945 },
      ...
      { g: new THREE.SphereGeometry(0.044, R_NUB, 10), c: P.runnerSkin,
        x: -0.048, y: HY - 0.300, z: 0.066, sx: 0.58, sy: 1.35, sz: 0.50, rz: -0.16 },
      { g: new THREE.SphereGeometry(0.044, R_NUB, 10), c: P.runnerSkin,
        x: 0.048, y: HY - 0.300, z: 0.066, sx: 0.58, sy: 1.35, sz: 0.50, rz: 0.16 },
      // ---- PHASE 1 OF THE OVERHAUL: A DESIGNED HEAD, NOT A SPHERE -------
      // One lathe profile, crown to chin: forehead carries the widest point,
      // the cheek pulls in under it, the jaw narrows deliberately toward a
      // chin that ends in a pole so the solid is closed on all sides. Depth
      // runs at 0.99 of width with the whole form shifted 0.012 forward,
      // which lands the front surface exactly on the eye planes at the eye
      // line -- the features sit ON the face instead of floating off a ball,
      // which was the brief's first complaint. Crown tops out at HY+0.280,
      // inside the old sphere's own 0.282, so no height contract can move.
      // The crown region is drawn INSIDE the cap shell on purpose, and it
      // was not the first time: the lathe's upper skull reached 0.228 at a
      // height where the cap's own radius is 0.220, so the scalp surfaced
      // through the fabric across the whole crown -- the coin-of-scalp bug
      // at full size. Everything above the brow ridge now clears the cap's
      // sphere (r 0.268 about HY+0.012) by at least 0.02; the face below the
      // band is where the width lives.
      { g: lathe([
          [0.001, 0.272],   // crown pole, under the cap's 0.280
          [0.130, 0.242],   // crown (cap radius there: 0.141)
          [0.192, 0.168],   // upper skull (cap: 0.220)
          [0.240, 0.080],   // upper forehead, tucking under the band
          [0.256, 0.030],   // brow ridge -- the widest the head gets
          [0.250, -0.040],  // temple easing in
          [0.238, -0.120],  // cheekbone
          [0.186, -0.200],  // jaw, narrowing with intent
          [0.102, -0.256],  // chin
          [0.001, -0.280],  // chin pole
        // ry pi: LatheGeometry carries its wrap seam at the start angle and
        // computeVertexNormals leaves a shading crease there -- it rendered
        // as a line straight down the middle of the face. The seam now faces
        // dead astern, under the hair, where a crease is a nape.
        ]), c: P.runnerSkin, y: HY, z: 0.012, sz: 0.99, ry: Math.PI },
      // ---- THE NOSE (the anatomy pass) --------------------------------
      // The owner, on the portrait: "He still does not look like a human."
      // This face had eyes, brows and a mouth and NO NOSE -- which no human
      // face survives from the front, and no reference character omits. A
      // small soft wedge under the eye line, proud of the skull by 0.024:
      // enough to catch the ramp and break the face's flatness, small enough
      // that from dead astern it is entirely inside the head's outline and
      // every silhouette contract holds by construction.
      { g: new THREE.SphereGeometry(0.049, R_NUB, 10), c: P.runnerSkin,
        y: HY - 0.064, z: 0.254, sx: 0.74, sy: 1.05, sz: 0.66 },
      // THE ANATOMY PASS'S EARS ARE GONE, because they had never been drawn.
      // It added a second pair at r 0.062 / x 0.244 without noticing the
      // original pair fifty lines down (r 0.080 / x 0.254, spanning x 0.2156
      // to 0.2924): the new discs sat ENTIRELY inside the old ones -- every
      // extent nested, to within 0.001 at the lobe -- which is the lace-panel
      // class of part, argued into place and never counted. The original pair
      // is the character's ears and always was.
      ...
      //
      // phiLength 0.42pi -> 0.3812pi, and it is not a taste change: it is the
      // brim lift below, propagated. The crown is the cap's FABRIC and it has
      // to die inside the band or its raw cut edge shows. At 0.42pi its lower
      // edge sat at HY + 0.072, which was comfortably inside a band spanning
      // HY + 0.023 to HY + 0.097 and is 0.007 under the underside of the band
      // once that moves to HY + 0.065. 0.3812pi puts the edge at HY + 0.105,
      // which is 0.040 inside the new band and 0.034 under its top.
      // ---- THE HAIR, AND THE CAP IS GONE -------------------------------
      // The owner's character sheet settles the headwear question the cap
      // was an answer to: he wears an OPEN-TOP VISOR, and the crown above it
      // is a spiky brown mop. The red shell that used to be the cap's fabric
      // is replaced by a hair base shell and a cluster of tilted lobes --
      // every tip inside HY+0.29 (crown budget: world 1.559 at rest, 1.599
      // at the top of the bob, under the 1.60 HEIGHT contract) and inside
      // the band's 0.294 half-width, so no silhouette number moves.
      // r 0.263, not 0.258: the skull's own pole is at HY+0.272 and a shell
      // whose pole lands on exactly the same height z-fights there -- the
      // coin-of-scalp bug's third appearance in this file. 0.263+0.014 puts
      // the hair 0.005 proud at the pole and more everywhere else.
      { g: new THREE.SphereGeometry(0.263, R_HEAD, 12, 0, Math.PI * 2, 0, Math.PI * 0.52), c: P.runnerHair, y: HY + 0.014 },
      // The mop: a big crown mass, two front spikes leaning over the brim
      // line, two swept back over the ears, one falling to the nape.
      { g: new THREE.SphereGeometry(0.105, R_NUB, 12), c: P.runnerHair,
        y: HY + 0.208, z: 0.018, sx: 1.15, sy: 0.72, sz: 1.05 },
      { g: new THREE.SphereGeometry(0.085, R_NUB, 10), c: P.runnerHair,
        x: -0.100, y: HY + 0.198, z: 0.112, sy: 0.80, rx: 0.42, rz: 0.48 },
      { g: new THREE.SphereGeometry(0.085, R_NUB, 10), c: P.runnerHair,
        x: 0.100, y: HY + 0.198, z: 0.112, sy: 0.80, rx: 0.42, rz: -0.48 },
      { g: new THREE.SphereGeometry(0.090, R_NUB, 10), c: P.runnerHair,
        x: -0.132, y: HY + 0.192, z: -0.085, sy: 0.85, rz: 0.55 },
      { g: new THREE.SphereGeometry(0.090, R_NUB, 10), c: P.runnerHair,
        x: 0.132, y: HY + 0.192, z: -0.085, sy: 0.85, rz: -0.55 },
      { g: new THREE.SphereGeometry(0.095, R_NUB, 10), c: P.runnerHair,
        y: HY + 0.182, z: -0.148, sy: 0.90, rx: -0.48 },
      { g: new THREE.SphereGeometry(0.060, R_NUB, 10), c: P.runnerHair,
        x: -0.040, y: HY + 0.238, z: 0.055, sy: 0.78, rz: 0.30 },
      { g: new THREE.SphereGeometry(0.060, R_NUB, 10), c: P.runnerHair,
        x: 0.055, y: HY + 0.236, z: -0.030, sy: 0.78, rz: -0.35 },
      ...
      // wrong.
      { g: new THREE.SphereGeometry(0.246, 24, 12, Math.PI * 1.5 - 1.34, 2.68, Math.PI * 0.44, Math.PI * 0.30), c: P.runnerHair, y: HY + 0.002, z: -0.030 },
      // ---- the fringe -------------------------------------------------
      // The reference character's single strongest identity cue is dark
      // hair showing UNDER the headwear -- at the temples in front, and in
      // front of the ears. Four wisps, all outboard of the brows (which own
      // x up to 0.16) and all inside the figure's half-width, so nothing
      // the lane or envelope measures can see them. Each stands 0.02-0.03
      // proud of the skull at its own height -- the arithmetic the knuckle
      // row got wrong -- so they are shapes, not decals.
      // Raised to meet the band's lower edge: hung lower they read as a
      // second pair of eyebrows rather than as hair meeting the visor.
      { g: new THREE.SphereGeometry(0.070, R_NUB, 10), c: P.runnerHair,
        x: -0.195, y: HY + 0.064, z: 0.140, sx: 0.60, sy: 0.48, sz: 0.50, rz: 0.35 },
      { g: new THREE.SphereGeometry(0.070, R_NUB, 10), c: P.runnerHair,
        x: 0.195, y: HY + 0.064, z: 0.140, sx: 0.60, sy: 0.48, sz: 0.50, rz: -0.35 },
      // Sideburns hug the skull (they floated off the cheek at the quarter
      // view at x 0.228 / z 0.065 -- proud of the shape at their own height).
      { g: new THREE.SphereGeometry(0.050, R_NUB, 10), c: P.runnerHair,
        x: -0.218, y: HY - 0.055, z: 0.042, sx: 0.32, sy: 0.80, sz: 0.42 },
      { g: new THREE.SphereGeometry(0.050, R_NUB, 10), c: P.runnerHair,
        x: 0.218, y: HY - 0.055, z: 0.042, sx: 0.32, sy: 0.80, sz: 0.42 },
      ...
      { g: new THREE.SphereGeometry(0.074, R_NUB, 12), c: P.runnerHair, y: HY - 0.228, z: -0.052, sx: 0.55, sy: 1.00, sz: 0.80 },
      ...
      //
      // ---- THE BRIM LIFT, AND IT IS THE ONE SILHOUETTE NUMBER THAT MOVED ---
      //
      // y HY + 0.060 -> HY + 0.102. The band's underside goes with it, from
      // HY + 0.023 to HY + 0.065, and that 0.042 is the whole of this change.
      //
      // WHY. This character had no forehead. The eye's sclera reaches
      // HY + 0.035 and the band's underside sat at HY + 0.023, so the brim was
      // 0.012 INTO the top of each eye and there was nowhere for a brow to be.
      // The pass that found that wrote it up as "what wearing a cap pulled
      // down means" -- a limitation dressed as an intention -- and the owner's
      // own quality bar, reference/ss-character-quality.png, refutes it: both
      // its characters have heavy dark brows on bare forehead BELOW the brim,
      // wearing the same hat, because their brim sits higher.
      //
      // The cost was named before it was paid: the hat moves, and the hat is
      // this character's identity. The owner was asked and chose to pay it.
      // The authorisation is for the BRIM SPECIFICALLY; every other number on
      // this figure is pinned exactly where it was, which is why the crown
      // shell above and the peak below both move by the same 0.042 rather than
      // being re-tuned, and why nothing else in this file moved at all.
      //
      // WHAT THE 0.042 BUYS, exactly: 0.0302 of bare forehead between the eye's
      // top and the band's underside. That is enough for a brow to EXIST and
      // very nearly not enough for one to MOVE -- see the brow's own note and
      // the expression block for what that forced.
      //
      // Nothing about the band's RADIUS changed, so the head's 0.294 contribu-
      // tion to half-width is untouched to the float; and the band's new top at
      // HY + 0.139 is still 0.143 under the cap's pole, so the crown of the
      // figure is untouched too. tools/envelope.js confirms both across all
      // eight states.
      // Red now, with the yellow dropped to a strip along its lower edge --
      // the sheet's visor is a red band over a yellow rim, not a yellow
      // sweatband. Same cylinder, same contracts.
      { g: new THREE.CylinderGeometry(0.272, 0.272, 0.070, R_HEAD), c: CAP, y: HY + 0.118 },
      { g: new THREE.CylinderGeometry(0.2735, 0.2735, 0.026, R_HEAD), c: TRIM, y: HY + 0.092 },
      ...
      { g: new THREE.CylinderGeometry(0.250, 0.250, 0.022, R_TORSO, 1, false, -Math.PI * 0.5, Math.PI), c: CAP,
        //
        // y + 0.042, carried by the brim lift. Every clearance in the note
        // above is stated RELATIVE TO THE BAND -- the plate rides at the
        // band's own mid-height with 0.024 of clear air over it and 0.028
        // under it -- so moving the two together preserves all of them
        // exactly, and re-deriving them would have been the mistake.
        //
        // CAP red rather than JACKET, and a second, slightly larger yellow
        // plate hung 0.015 beneath it: the sheet's brim is red on top with
        // a yellow underside that shows as a rim from every angle below the
        // eye line -- which the chase and portrait cameras both are.
        y: HY + 0.104, z: 0.080, rx: 0.06, sz: 1.40 },
      { g: new THREE.CylinderGeometry(0.256, 0.256, 0.012, R_TORSO, 1, false, -Math.PI * 0.5, Math.PI), c: TRIM,
        y: HY + 0.089, z: 0.080, rx: 0.06, sz: 1.42 },
    ...
    //
    // On its own pivot AT THE EYE LINE, which is the whole of what makes
    // expression possible later: a mesh welded in head space scales about the
    // HEAD's origin, so narrowing the eyes by scaling y would slide them down
    // the face at the same time. The pivot puts the origin between them.
    const EYE_PIVOT_Y = HY - 0.016;
    const eyePivot = pivot(head, 0, EYE_PIVOT_Y, 0);
    const eyes = new THREE.Mesh(weld(eyePieces()), S.flat(0xffffff, { vertexColors: true }));
    eyePivot.add(eyes);

    // ---- PHASE 2 OF THE OVERHAUL: EYELIDS -------------------------------
    //
    // The brief: "The upper eyelids are extremely important. They should
    // give the character personality... The normal gameplay expression
    // should feel focused + confident + energetic."
    //
    // Two skin shells on their own pivot AT THE EYES' TOP EDGE, tilted a
    // few degrees down toward the nose, resting a third of the way over the
    // eye -- which is what reads as focus; a fully open circle reads as
    // alarm. The same shells ARE the blink: the pivot's y-scale grows them
    // down over the eye, so a blink is a lid closing rather than an eyeball
    // squashing, and the old scale-squash drops to a supporting squint.
    // Geometry is translated so the TOP edge sits at the pivot's origin --
    // a lid must descend from its lash line, not inflate about its middle.
    //
    // A subtle static LOWER lid rides in the same weld at each eye's foot:
    // the brief asks for it, and it seats the eye in the face.
    const LID_TINT = 0xeab183;   // skin a step down: the lid shades its eye
    // CURVED SHELLS, not boxes. The first cut used flat slabs at a fixed z
    // and the three-quarter view called it immediately: the outer edge of
    // the far lid floated off the cheek like a visor of skin. A lid is a
    // contact-lens slice of the face's own curvature -- a sphere cap a hair
    // wider than the eye, its shell just proud of the eye planes, so it
    // wraps AND occludes from every angle the portrait or the finish orbit
    // can take.
    const lidPivot = pivot(head, 0, EYE_PIVOT_Y + 0.0512, 0);
    const lids = new THREE.Mesh(weld([
      { g: new THREE.SphereGeometry(0.086, 16, 10, 0, Math.PI * 2, 0, Math.PI * 0.52),
        c: LID_TINT, x: -0.100, y: -0.052, z: 0.185, sz: 0.70, sy: 0.68, rz: -0.10 },
      { g: new THREE.SphereGeometry(0.086, 16, 10, 0, Math.PI * 2, 0, Math.PI * 0.52),
        c: LID_TINT, x: 0.100, y: -0.052, z: 0.185, sz: 0.70, sy: 0.68, rz: 0.10 },
    ]), S.flat(0xffffff, { vertexColors: true }));
    lidPivot.add(lids);
    const lowerLids = new THREE.Mesh(weld([
      { g: new THREE.SphereGeometry(0.080, 14, 8, 0, Math.PI * 2, Math.PI * 0.78, Math.PI * 0.22),
        c: LID_TINT, x: -0.100, y: EYE_PIVOT_Y + 0.010, z: 0.186, sz: 0.70, sy: 0.85 },
      { g: new THREE.SphereGeometry(0.080, 14, 8, 0, Math.PI * 2, Math.PI * 0.78, Math.PI * 0.22),
        c: LID_TINT, x: 0.100, y: EYE_PIVOT_Y + 0.010, z: 0.186, sz: 0.70, sy: 0.85 },
    ]), S.flat(0xffffff, { vertexColors: true }));
    head.add(lowerLids);
    const LID_REST = 0.34;
    lidPivot.scale.y = LID_REST;
    ...
    // The brows get their own pivot for the same reason the eyes did: they must
    // NOT ride the eye's aperture scale. Narrowing an eye does not squash the
    // brow above it, and welding the two into one mesh to save a draw call
    // would have tied them together permanently.
    // Raised 0.016 in the overhaul: the new upper lids reach HY+0.058 and
    // the brows were being swallowed by the very shells built to sit under
    // them. The brow must always clear the lid -- it is the loudest line on
    // the face and the one the expression system leans on hardest.
    const browPivot = pivot(head, 0, HY + 0.0600, 0);
    const brows = new THREE.Mesh(weld(browPieces()), S.flat(0xffffff, { vertexColors: true }));
    browPivot.add(brows);
    const browGeo = browRig(brows);

    // The pivot is at the UPPER LIP, not at the middle of the mouth, so the
    // aperture opens downward the way a jaw does. A mouth scaled about its own
    // centre grows upward into the nose as well, which reads as a shout rather
    // than as breathing.
    const mouthPivot = pivot(head, 0, HY - 0.127, 0.196);
    ...
    for (const side of [-1, 1]) {
      ...
      // ---- PHASE 5, THE LEG AS ONE DRAWN SURFACE ------------------------
      // Capsule plus hamstring ball becomes one lathe: full through the quad
      // and hamstring, tapering above the knee. The widest point keeps the
      // capsule's 0.114 so the lane-width arithmetic sees the same thigh.
      const thigh = multi([
        { g: lathe([
            [0.001, 0.172],
            [0.088, 0.128],
            [0.114, 0.040],   // quad/hamstring belly -- the capsule's own r
            [0.104, -0.050],
            [0.094, -0.120],  // arriving at the knee ball still tapering
            [0.001, -0.164],
          ], null, { seg: R_LIMB }), c: P.runnerSkin, ry: Math.PI },
      ]);
      ...
      const shin = multi([
        // One surface from under the knee to the sock: calf belly high and
        // aft-of-nothing (a lathe is symmetric -- the belly's 0.100 sits
        // inside the old calf ball's own 0.101), tapering hard to an ankle
        // half its width, which is the taper that makes a shin a shin.
        { g: lathe([
            [0.001, 0.070],
            [0.078, 0.038],
            [0.100, -0.048],  // the calf's belly
            [0.082, -0.108],
            [0.068, -0.160],  // inside the sock
            [0.058, -0.205],
            [0.001, -0.224],
          ], null, { seg: R_LIMB }), c: P.runnerSkin, ry: Math.PI },
        ...
        // (The calf ball is folded into the lathe's own belly above.)
        // True white and taller: the sheet's crew socks are a feature, not
        // a cuff. Bottom edge kept at -0.161, deep inside the shoe collar.
        { g: new THREE.CylinderGeometry(0.110, 0.102, 0.100, R_LIMB), c: 0xf7f5ee, y: -0.111 },
      ]);
      knee.add(shin);
      ...
      const foot = multi([
        // The ankle opening (Phase 7): a real trainer has a padded collar the
        // leg disappears INTO, and its absence is what made these read as
        // white blobs the legs rested on. A short dark ring at the top rear,
        // inside the box's own footprint (0.092 against the box's 0.095 and
        // the stripe's 0.098), so no silhouette number moves.
        { g: new THREE.CylinderGeometry(0.084, 0.096, 0.055, R_LIMB), c: SHOE_DK, y: 0.012, z: -0.020 },
        // Chamfer 0.030 -> 0.040: a third of the shoe's height rounded off,
        // which is what moves the upper from "brick with domes" toward the
        // puffy trainer volume every reference character wears. Extents are
        // 0.185 x 0.105 -- INSIDE the old 0.190 x 0.110 -- so the stripe and
        // outsole below keep owning the slide silhouette untouched.
        { g: roundedBox(0.185, 0.105, 0.230, 0.040, R_NUB), c: P.runnerShoe, y: -0.033, z: 0.038 },
        ...
        // White now, per the sheet: the midsole line is white over the pale
        // outsole, and the YELLOW moved up into the side panels below --
        // same geometry to the float, so the slide's half-width (which this
        // piece's front corner owns) is untouched.
        { g: new THREE.BoxGeometry(0.196, 0.028, 0.240), c: 0xf2efe6, y: -0.100, z: 0.036 },
        // The sheet's yellow side flashes, inset 0.0065 from the stripe's
        // own half-width so they can never take the silhouette from it.
        { g: new THREE.BoxGeometry(0.008, 0.048, 0.130), c: TRIM, x: -0.0915, y: -0.052, z: 0.030 },
        { g: new THREE.BoxGeometry(0.008, 0.048, 0.130), c: TRIM, x: 0.0915, y: -0.052, z: 0.030 },
      ...
      // elbow with a line round it.
            // ---- SLIMMED AND SHORTENED IN THE ANATOMY PASS ---------------------
      // The owner, on the portrait: "He still does not look like a human."
      // From the front the arms were the loudest reason: bead-columns nearly
      // a head wide, hanging past the shorts. Every radius here came down
      // ~18% and the elbow rose 0.026, which brings the wrists up to the
      // hip line where a standing human's wrists are. Slimming only ever
      // SHRINKS the figure's half-width and RAISES its lowest point, so
      // both directions are the safe side of every silhouette contract --
      // and tools/envelope.js re-measures them rather than trusting this.
      // ---- PHASE 5, THE ARM AS ONE DRAWN SURFACE ------------------------
      // The upper arm was a capsule with a deltoid ball and a triceps ball
      // welded on -- three overlapping primitives whose intersection curves
      // the ramp shaded as seams, which is the "collection of primitive
      // shapes" the overhaul brief opens with. One lathe now carries the
      // whole segment: full at the deltoid, tapering through the triceps to
      // the elbow, the way an arm actually narrows. Its widest point holds
      // the old deltoid ball's 0.092, so the abducted jump silhouette that
      // tools/envelope.js measures sees the same arm it measured before.
      const upper = multi([
        { g: lathe([
            [0.001, 0.048],   // shoulder pole, buried in the vest's cap
            [0.070, 0.020],
            [0.092, -0.045],  // deltoid -- the old ball's own radius
            [0.079, -0.110],
            [0.068, -0.175],
            [0.060, -0.228],  // arriving at the elbow still tapering
            [0.001, -0.250],
          ], null, { seg: R_LIMB }), c: P.runnerSkin, ry: Math.PI },
        // The tee's SLEEVE, on the arm so it swings with it. The old base-
        // layer sleeve was cut for reading as a pale hole in the vest; this
        // one is the tee's own red, so that failure mode does not exist --
        // and the sheet's tee has real short sleeves ending mid-upper-arm.
        // 0.099 over the lathe's 0.092 deltoid: a garment standing 0.007
        // proud, with the trunk's red shoulder cap covering the joint above.
        { g: new THREE.SphereGeometry(0.099, R_LIMB, 12), c: P.runnerVest, y: -0.020, sy: 0.72 },
        { g: new THREE.CylinderGeometry(0.099, 0.091, 0.110, R_LIMB), c: P.runnerVest, y: -0.062 },
        { g: new THREE.CylinderGeometry(0.093, 0.089, 0.020, R_LIMB), c: JACKET, y: -0.118 },
        // The old deltoid/triceps notes, kept for the record of the pass
        // that placed them; the masses live in the profile above now.
        //
      ...
      // never shows.
      const elbow = pivot(shoulder, 0, -0.236, 0);
      ...
      const fore = multi([
        // The same one-surface argument as the upper arm: capsule plus a
        // welded belly ball read as two objects; a lathe that is thickest
        // just under the elbow and tapers to the wrist reads as a forearm.
        // 0.074 at the belly against the old ball's ellipsoid 0.067 in x --
        // radial symmetry costs 0.007 of x there, bought back from a z that
        // was already 0.078. The wrist dies into the band at -0.138.
        { g: lathe([
            [0.001, 0.052],
            [0.066, 0.015],
            [0.074, -0.045],  // the forearm's belly
            [0.064, -0.100],
            [0.054, -0.138],  // the wrist, inside the band
            [0.001, -0.158],
          ], null, { seg: R_LIMB }), c: P.runnerSkin, ry: Math.PI },
        ...
        { g: new THREE.SphereGeometry(0.072, R_NUB, 12), c: P.runnerSkin, y: -0.012, sy: 0.92, sz: 0.96 },
        ...
        // (The forearm-belly ball this note argued for is folded into the
        // lathe profile above -- the argument survives, the extra outline
        // does not.)
        ...
        { g: new THREE.CylinderGeometry(0.082, 0.082, 0.046, R_LIMB), c: TRIM, y: -0.158 },
        // ---- the hand ------------------------------------------------------
        //
        ...
        { g: new THREE.SphereGeometry(0.088, R_LIMB, 16), c: MITT, y: -0.226, z: 0.012, sy: 0.78, sz: 0.84 },
        ...
        { g: new THREE.SphereGeometry(0.034, R_NUB, 12), c: MITT_DK, x: -0.042, y: -0.264, z: -0.048 },
        { g: new THREE.SphereGeometry(0.037, R_NUB, 12), c: MITT_DK, x: 0.000, y: -0.265, z: -0.048 },
        { g: new THREE.SphereGeometry(0.033, R_NUB, 12), c: MITT_DK, x: 0.042, y: -0.263, z: -0.048 },
        // ---- the curled finger mass (Phase 6) ----------------------------
        // The knuckle row above serves the chase camera; this serves the
        // portrait and the finish orbit, which look at the FRONT of the hand
        // and found a featureless ellipsoid there. One flattened lobe under
        // the palm's leading edge -- the mass of four curled fingers, not
        // four fingers -- in the hand's own skin so it reads as form, not
        // trim. Its lowest point is -0.294 against the mitt's own -0.295,
        // so the hand's reach (the jump silhouette's term) is untouched.
        { g: new THREE.SphereGeometry(0.052, R_NUB, 12), c: MITT,
          y: -0.262, z: 0.030, sx: 1.30, sy: 0.62, sz: 0.80 },
        // ---- the thumb ---------------------------------------------------
        //
        ...
        { g: new THREE.CapsuleGeometry(0.036, 0.042, 4, 12), c: MITT,
          x: -side * 0.074, y: -0.202, z: -0.036, rx: 0.30, rz: side * 0.62 },
        ...
        // Square, per the sheet: a smartwatch case with a rounded-rect
        // screen, riding the outboard face of the yellow band.
        ...(side === -1 ? [
          { g: roundedBox(0.020, 0.044, 0.038, 0.008, 12), c: 0x1a2038,
            x: side * 0.088, y: -0.158 },
          { g: roundedBox(0.007, 0.032, 0.027, 0.003, 8), c: 0x9fe8d8,
            x: side * 0.0985, y: -0.158 },
        ] : []),
      ]);
      elbow.add(fore);
    ...
    const api = {
      ...
      height: HEIGHT,
      shadow,
      // eyes and mouth are exported for the same reason every other pivot
      // here is: tools/pose-diff.js compares joints BY NAME, and a joint the
      // api does not name is a joint no A/B can check. The expression terms
      // are the newest thing on this rig and therefore the ones most in need
      // of it.
      parts: { body, hips, spine, chest, neck, head, hood: hoodPivot,
        eyes: eyePivot, brows: browPivot, mouth: mouthPivot, legs, arms },
      phase: 0,
    };
    ...
    api.update = function (dt, st) {
      ...
      // ---- weight events (the animation-quality pass) ---------------------
      // THE LANDING. air01 collapsing to zero is the touchdown; the impulse
      // it throws drives knee give, a hip drop, a chest nod and a one-beat
      // squash, all decaying over ~0.25s -- the "feet contact, knees
      // compress, hips drop, torso follows" chain the brief spells out.
      // Edge-triggered, so the held airborne pose tools/envelope.js measures
      // can never see it, and eased so it arrives and leaves without a tick.
      if (airPrev > 0.25 && air <= 0.02) landK = 1;
      airPrev = air;
      landK = Math.max(0, landK - dt * 4.2);
      const landE = sm(landK) * (1 - slid);
      // SQUASH AND STRETCH, about the FEET (the body pivot sits at ground
      // level, so scale.y compresses toward the road instead of lifting the
      // shoes off it). Airborne the figure stretches 3.5%; on the landing
      // beat it squashes 5%. Volume is roughly kept by counter-scaling the
      // other two axes at half rate. Both terms are zero in every held
      // silhouette state -- spread's stretch is the one exception, and it
      // only ever RAISES the airborne crown, which no ground contract reads.
      const sq = 1 + spread * 0.035 - landE * 0.050;
      body.scale.y = sq;
      body.scale.x = body.scale.z = 1 - (sq - 1) * 0.5;
      // THE LANE-CHANGE CHAIN. leanC lags the bank at ~7/s; leanN chases it
      // on an underdamped spring (omega 14, zeta 0.5) so the head settles
      // with one small overshoot when the body arrives in the new lane.
      leanC += (lean - leanC) * (1 - Math.pow(0.001, dt));
      leanNV += ((lean - leanN) * 196 - leanNV * 14) * hdt;
      leanN += leanNV * hdt;
      ...
      for (let i = 0; i < legs.length; i++) {
        ...
        // Natural asymmetry (the brief: "both sides should not move
        // identically"): one side runs 2% hot and the other 2.5% cool.
        ...
        L.hip.rotation.x = hipRunA * cyc + tuck * 0.70
          - slid * (lead ? 1.80 : 1.66);
        // landE folds the knee an extra 0.30 on the landing beat -- the
        // knees absorbing the drop before the cycle takes back over.
        L.knee.rotation.x = 0.18 + kneeRunA * cyc + tuck * 1.25 + landE * 0.30
          + slid * (lead ? 0.02 : 0.62);
        ...
        const ankFlat = 0.16 - (hipRunA + kneeRunA + 0.18);
      ...
      // landE: the hips drop 0.05 on the touchdown beat and recover -- the
      // half of landing compression the silhouette reads. Downward only, so
      // the crown-at-top-of-bob contract cannot be touched.
      body.position.y = bob + air * 0.09 - landE * 0.05;
      ...
      // The lane-change chain (see the filters above). During the bank's
      // onset (lean - leanC) is signed toward the new lane: the HIPS take an
      // extra share immediately (they lead), the CHEST runs behind by the
      // lag (it trails the spine's own bank), and both settle to exactly
      // zero once the bank is held -- a held lean is untouched, only the
      // ARRIVAL is choreographed.
      hips.rotation.z += -(lean - leanC) * 0.12;
      chest.rotation.z += (lean - leanC) * 0.10;
      ...
      chest.rotation.x = POLISH * 0.055 * (plantW - 0.156) * cycD
        // ...plus the landing nod: the torso follows the hips down on the
        // touchdown beat and straightens as the impulse decays.
        + landE * 0.15;
      ...
      // The head is last in the lane-change chain: it trails on the spring
      // copy, so it arrives after the chest and settles with one small
      // overshoot -- the "settling motion when the character reaches the new
      // lane" the brief asks for, on the one joint whose settle a viewer
      // actually watches.
      neck.rotation.z += (lean - leanN) * 0.16 * (1 - slid);
      ...
      if (stand > 0) {
        ...
        for (let i = 0; i < arms.length; i++) {
          ...
          // 0.24, from 0.17: at 0.17 the hands hung dead in front of the
          // thighs and the tones merged (see MITT). A standing runner's
          // hands ride just clear of the hips; the silhouette gets daylight
          // between arm and leg, which is most of what makes two limbs read
          // as two limbs from the front.
          to(A.shoulder, 'z', A.side * (0.24 + 0.02 * sway * A.side), stand);
      ...
      // ---- the vest hem and the short cuff ---------------------------------
      //
      // Same shape as the bib above and for the same reasons, with two
      // differences that matter.
      //
      // BOTH ARE SCALED BY POLISH, where the bib is not. That is deliberate:
      // the bib's flutter is part of the character as it stood before the
      // animation work, and these are secondary motion in exactly the sense
      // stage 2 defined, so putting them inside the A/B is what keeps
      // ?polish=0 meaning "the cycle before any of this". It also makes the
      // identity proof trivial and exact rather than approximate -- at POLISH
      // 0 the amplitude is identically zero, the deformation is identically
      // the rest pose, and every vertex is bit-for-bit where it was.
      //
      // THE IMPACT TERM. `bobV` is the collar's own vertical rate, already
      // computed and clamped for the hood. Only its DOWNWARD half is used:
      // cloth flies out when the body drops onto a foot and does nothing
      // special when it rises, so a symmetric term would put a flick at both
      // ends of the stride and read as a wobble rather than as a footfall.
      // This is the whole reason the hems beat with the run rather than
      // floating free the way the bib does -- the bib is pinned to a back and
      // sees the body's motion only through the panel, a hem is at the waist
      // and takes the landing directly.
      //
      // THE AMPLITUDES ARE SMALL and they are meant to be. 0.016 to 0.026 is
      // two to three pixels at the 121-131 px per unit the shipped frame gives
      // this figure -- but a hem is a hard value edge two hundred pixels long,
      // and what the eye resolves on an edge that size is not the displacement,
      // it is the WAVE running along it. Anything past about 0.04 stops being
      // a hem lifting and becomes a skirt.
      //
      // Two lobes on the vest and three on the shorts, so the waves have
      // different spatial periods as well as different clocks. Directions
      // differ too: the vest's clock runs forward and the short's backward, so
      // the two waves cross rather than chase.
      const drop = Math.max(0, -bobV);
      vestT += dt * (4.4 + 5.2 * sp01);
      shortT -= dt * (5.6 + 6.0 * sp01);
      hemFlutter(vestHem,
        POLISH * ((0.016 + 0.010 * sp01) * (1 - slid * 0.60) + spread * 0.008 + drop * 0.010),
        vestT, 2, 0.55);
      hemFlutter(shortHem,
        POLISH * ((0.014 + 0.008 * sp01) * (1 - slid * 0.45) + spread * 0.006 + drop * 0.012),
        shortT, 3, 0.45);

      // ---- expression --------------------------------------------------------
      //
      // The owner's last item, and the one where it is easiest to build
      // something nobody will ever see. So it is worth writing down exactly
      // where this is and is not visible before writing any of it:
      //
      //   IN PLAY, NOWHERE. The chase camera is dead astern, the face owns 0
      //     pixels in 48 of 48 frames through it (tools/resolve.js), and a
      //     slide deliberately yaws the head FURTHER away because a face
      //     turned to the lens reads as a fall. Nothing below is drawn during
      //     a race.
      //   AT THE START PANEL, THE FINISH AND THE ORBIT SHEET, ALL OF IT.
      //
      // Which makes this cheap rather than pointless: two scale writes on two
      // pivots, no geometry, no draw call, no allocation. It is the least
      // expensive item on the whole list and it is the one that decides whether
      // the face reads as a face or as a mask.
      //
      // WHAT DRIVES IT. Effort, assembled from the three things this file
      // already knows and none of which it had a use for on the face: pace
      // across the honest band, the climb, and the surge stage 3 built. Not
      // fuel -- main.js does not hand this file fuel, and inventing a channel
      // for a term this small would be the tail wagging the dog.
      //
      // WHAT IT DOES. The mouth opens and the eyes narrow, which is the pair
      // every reference frame of a running character has, and they are driven
      // TOGETHER off one signal so they cannot disagree. There are no brows to
      // drive: the cap band's underside sits 0.017 into the top of the eye and
      // this character's brow line is inside his hat, which is what wearing a
      // cap pulled down means. The aperture is the brow.
      //
      // A WINCE, separately and on top. Any of the three contact signals --
      // the wall bounce, the kerb trip, the generic stumble -- squeezes the
      // eyes and pulls the mouth wide. This is the one expression term with a
      // gameplay argument behind it as well as an art one: a contact is the
      // event that ends a record attempt, and the replay and the finish are
      // where a player looks at what happened.
      //
      // Everything is scaled by POLISH and every factor is exactly 1 at
      // POLISH 0, so the A/B build shows the face he had before this.
      const effort = Math.max(0, Math.min(1,
        0.30 + 0.52 * sp01 + 0.85 * Math.max(0, grade) + 0.55 * surge));
      const wince = Math.max(stumble, trip, knock);
      // ---- THE BROW IS THE PRIMARY CHANNEL, AND THE APERTURE WAS FIGHTING IT
      //
      // This block used to drive the eye's aperture alone, because there were
      // no brows and this file said there could not be. There are now, so the
      // honest question is not "add the brow too" but which of the two carries
      // the expression -- and that was settled by counting pixels.
      //
      // Both channels were isolated in a patched build and the head rendered at
      // effort 0 against effort 1 through the face lens, counting the pixels
      // that CHANGE:
      //
      //     brow alone   11668        eye alone    3900        both    8585
      //
      // The brow wins 3.0x, and the reason is structural rather than a matter
      // of gains: an aperture can only shrink a shape the eye is already
      // looking at, while a brow ARRIVES over the top of the iris and changes
      // the eye's outline. A shape that moves into a region beats a shape that
      // shrinks inside one.
      //
      // BUT LOOK AT THE THIRD NUMBER. Both together read LESS than the brow on
      // its own -- 8585 against 11668 -- so the second channel was not seconding
      // the first, it was DELETING 3083 pixels of it. A symmetric aperture
      // pulls the eye's top edge down and away exactly as the brow comes down
      // to meet it, so the brow lands on skin instead of on eye and neither
      // mark gets its contrast. Two correct animation terms, each defensible on
      // its own, subtracting.
      //
      // The fix is not smaller gains, it is a different EDGE. The aperture now
      // closes from the BOTTOM: the pivot is translated by half the height it
      // loses, which pins the eye's top edge exactly where it was and raises
      // the lower lid instead. The brow then owns the top of the eye and the
      // lid owns the bottom, they never compete for the same pixels, and the
      ...
      // ---- gameplay face beats (the animation-quality pass) ----------------
      // The brief's core demand for the face: IT CHANGES WITH THE ACTION.
      // Two transient states join the effort/wince drive:
      //   shock   a contact or a trip -- brows fly up and level, lids snap
      //           open, the eye widens past rest (the one moment the
      //           "narrowing only" rule below is deliberately broken: he IS
      //           surprised), the mouth drops open.
      //   excite  airborne -- brows up a touch, mouth opens into the jump.
      // Both are driven by inputs that are identically zero in every held
      // measured state, so the envelope and the A/B cannot see them.
      const shock = Math.min(1, trip * 1.6 + knock * 1.3);
      const excite = spread;
      ...
      const browLift = mix(mix(-POLISH * (0.0035 * effort + 0.0030 * wince),
        ...
        // Shock throws the brows UP well past the composure range; a jump
        // lifts them a third of that. Both transients, both zero at rest.
        + POLISH * (0.0085 * shock + 0.0030 * excite);
      const browTilt = mix(mix(POLISH * (0.0045 * effort + 0.0030 * wince),
        POLISH * 0.0008, standF),
        POLISH * mix(0.0060, -0.0010, celJoy), celF)
        // ...and levels them: raised-and-flat is surprise, raised-and-knit
        // is strain, and a collision is the former.
        - POLISH * 0.0038 * shock;
      {
        const ba = browGeo.pos.array, br = browGeo.rest, bu = browGeo.u;
        for (let i = 0; i < browGeo.pos.count; i++) {
          ba[i * 3 + 1] = br[i * 3 + 1] + browLift + browTilt * bu[i];
        }
        browGeo.pos.needsUpdate = true;
      }
      ...
      const eyeShut = Math.min(0.94, mix(mix(POLISH * (0.16 * effort + 0.34 * wince),
        ...
        // Shock overrides the squint: the lids snap OPEN on a contact.
        * (1 - shock * 0.85);
      // The LID carries the closure now (phase 2 of the overhaul): from a
      // third-covered rest to fully drawn at eyeShut 1. The eyeball keeps a
      // quarter of the old squash as a squint, so effort still narrows the
      // eye underneath the descending lid instead of the two fighting.
      // Shock also lifts the lid PAST its rest coverage -- wide eyes.
      lidPivot.scale.y = (LID_REST + (2.05 - LID_REST) * eyeShut) * (1 - shock * 0.45);
      eyePivot.scale.y = 1 - eyeShut * 0.25 + POLISH * 0.07 * shock;
      ...
      // whole closure on the bottom one. At POLISH 0 eyeShut is identically
      // zero, so the scale is 1 and the offset is 0 -- the A/B is untouched.
      eyePivot.position.y = EYE_PIVOT_Y + EYE_SEMI * eyeShut * 0.25;
      ...
      mouthPivot.scale.y = mix(mix(1 + POLISH * (0.55 * effort + 0.30 * wince),
        1 - POLISH * 0.62, standF),
        1 + POLISH * mix(0.95 + 0.35 * celBreath, 0.70, celJoy), celF)
        // A contact drops the jaw; a jump opens the mouth into the leap.
        + POLISH * (0.85 * shock + 0.22 * excite);
      mouthPivot.scale.x = mix(mix(1 + POLISH * (0.10 * effort + 0.22 * wince),
        1 + POLISH * 0.24, standF),
        1 + POLISH * mix(0.18, 0.52, celJoy), celF)
        + POLISH * (0.10 * excite - 0.14 * shock);
