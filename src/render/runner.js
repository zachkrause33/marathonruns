 ...
 *   THIS CHARACTER HAS NO BROW LINE, AND THAT IS A CONSEQUENCE RATHER THAN A
 *     STYLE. The cap band's underside sits at HY + 0.023 and the eye's sclera
 *     reaches HY + 0.035, so the band is 0.012 INTO the top of each eye and
 *     there is no forehead at all. The aperture is therefore the brow, which is
 *     why expression drives the eye's own scale.
 *
 *     This was first written here as "what wearing a cap pulled down means",
 *     i.e. as a design choice being honoured. It is not. The owner's actual
 *     quality bar -- reference/ss-character-quality.png, which was a
 *     mislabelled screenshot of THIS GAME until another agent refiled it --
 *     gives both its characters heavy dark brows, drawn on bare forehead
 *     BELOW the cap brim. The reference wears the same hat and still has room,
 *     because its brim sits higher.
 *
 *     The number, so the next pass does not have to derive it: the band's
 *     underside has to rise 0.042 -- band centre HY + 0.060 to HY + 0.102 --
 *     to open a brow line with one ink width of clearance above the eye. That
 *     moves the hat, and the hat is the character's identity, so this pass did
 *     not do it. It is the owner's call and it is the one item on their list
 *     that could not be delivered without touching something they ruled out.
...
MR.Runner = (function () {
  ...
  function create() {
    ...
    const eyePivot = pivot(head, 0, HY - 0.016, 0);
    ...
    const api = {
      ...
      parts: { body, hips, spine, chest, neck, head, hood: hoodPivot,
        eyes: eyePivot, mouth: mouthPivot, legs, arms },
      phase: 0,
    };
    ...
    api.update = function (dt, st) {
      ...
      eyePivot.scale.y = 1 - POLISH * (0.20 * effort + 0.42 * wince);
