...
MR.Skin = (function () {
  ...
  function rigFromSkeleton(gltf, parts, mat) {
    ...
    for (const A of parts.arms) {
      ...
      // The 1.05 cap is the jump: the driver spreads the arms 1.56rad for
      // the airborne silhouette, and at that abduction the sculpt's armpit
      // weights stretch into visible webbing -- the owner's phone report
      // was "his arms are attached to him when he jumps". Capping the
      // TRANSPLANTED angle leaves the run's swing untouched (it never
      // exceeds ~0.8rad) and stops the jump short of the webbing zone;
      // the driver's own silhouette, which gameplay reads, is unchanged.
      add(A.shoulder, M['clav' + A.side] ||
        (M['clav' + A.side] = findSide(A.side, ['clavicle', 'shoulder'])), 0.40, 1.05);
      add(A.shoulder, M['upper' + A.side], 1, 1.05);
    ...
    function sync() {
      ...
      for (const pr of pairs) {
        ...
        if (pr[3]) {
          const ang = 2 * Math.acos(Math.min(1, Math.abs(_delta.w)));
          if (ang > pr[3]) _delta.slerp(_IDENT, 1 - pr[3] / ang);
        }
