...
MR.World = (function () {
   ...
   * Per-setting bridges then take what is left: stoneArch reaches |x| 67.5,
  ...
  function create(course) {
    ...
    const duckGeo = merge([
      ...
      gl(bxAt(0.26, 3.30, 0.30, -1.20, 1.65, 0, 0x37d6ff), GLOSS.paint),
      gl(bxAt(0.26, 3.30, 0.30, 1.20, 1.65, 0, 0x37d6ff), GLOSS.paint),
    ...
    const duckBoomGeo = (function () {
      const parts = [
        ...
        gl(hcbx(2.30, 0.22, 0.52, 0, 1.72, 0, 0x37d6ff, 0.05), GLOSS.paint),
        gl(hcbx(2.36, 0.09, 0.54, 0, 1.785, 0, 0xd8f8ff, 0.03), GLOSS.trim),
      ];
      // The skirt. Alternating cream and deep cyan, which is the boom's real
      // marking and is also the cheapest way to put a repeating vertical
      // rhythm in the one band the player is looking at.
      for (let i = 0; i < 7; i++) {
        const x = -0.90 + i * 0.30;
        parts.push(gl(bxAt(0.13, 0.18, 0.22, x, 1.52, 0, i % 2 ? 0x1f9fd0 : 0xd8f8ff), GLOSS.trim));
      }
      for (const sx of [-1, 1]) {
    ...
    const duckPipeGeo = (function () {
      ...
      const parts = [
        ...
        gl(cyl(0.19, 0.19, 2.30 * LANE_FIT, 14, 0, 1.62, 0, 0x37d6ff, 0, 0, Math.PI / 2), GLOSS.paint),
      ];
      ...
      for (const sx of [-1, 1]) {
        ...
        parts.push(gl(cyl(0.16, 0.16, 3.10, 10, sx * px, 1.55, 0, 0x37d6ff), GLOSS.paint));
      ...
      parts.push(gl(cyl(0.24, 0.24, 0.07, 14, px, 2.74, -0.17, 0xd8f8ff, Math.PI / 2), GLOSS.chrome));
      parts.push(gl(cyl(0.08, 0.08, 0.22, 8, px, 2.74, -0.08, 0x1f9fd0, Math.PI / 2), GLOSS.trim));
      // The elbow, left standard: a stub rising off the column and turning
      // back on itself. Thin, and out at the post, per the rule.
      parts.push(gl(cyl(0.13, 0.13, 0.54, 10, -px, 2.90, 0, 0x1f9fd0), GLOSS.paint));
      parts.push(gl(cyl(0.13, 0.13, 0.30, 10, -px + 0.15, 3.14, 0, 0x1f9fd0, 0, 0, Math.PI / 2), GLOSS.paint));
      return merge(parts);
    })();
     ...
     * measurable claim, and it was being answered by eye. variantIndex is a
     * pure function of (gate.z, lane) and the kind's draw bag, so the whole
     * course's casting is knowable up front -- this returns it, and the
     * question "how often does the same variant appear twice inside one visible
     * window" becomes arithmetic rather than a screenshot.
     *
     * It reports what actually SPAWNS, drawn through the weighted bag, so a
     ...
     * different game.
     */
    api.variantPlan = function (gates) {
      const bags = {};
      const probe = {};
      probe[K.JUMP] = jumpPool; probe[K.DUCK] = duckPool; probe[K.BLOCK] = blockPool;
      for (const k of Object.keys(probe)) {
        // The bag lives on the pooled object, so borrow one and give it back.
        const o = probe[k].claim();
        bags[k] = o.userData.bag;
        probe[k].release(o);
      }
      const out = [];
      for (const gate of gates) {
        for (let l = 0; l < 3; l++) {
          ...
          out.push({ z: gate.z, lane: l, kind: kind, variant: variantIndex(bags[kind], gate, l) });
        }
      }
      return out;
    };
