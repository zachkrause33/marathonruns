...
MR.World = (function () {
  ...
  function create(course) {
     ...
     * of the painted area.
     *
     * BE HONEST ABOUT WHAT THIS BUYS: a bit under half the gap. The rest is not
     * in the palette of the surfaces we have -- it is in the fact that their
     * play surface is a crimson train and ours is a road, 40% of the band is
     * tarmac the gate will not let us colour, and closing the remainder means
     * putting more saturated NON-ROAD area into the bottom third at a draw-call
     * price. That is a separate argument on separate evidence.
     */
    const SEAM_RAIL = overRoad(0x846fda, 1.28);
    const SEAM_BEAD = overRoad(0xfff6d8, 1.50);   // HELD -- see the hue note above
    const EDGE_LINE = overRoad(0xadb2d7, 1.80);
    ...
    const paintGeo = (function () {
      ...
      for (let i = 0; i < nJ; i++) {
        ...
        parts.push(part(new THREE.PlaneGeometry(K.TRACK_HALF_WIDTH * 2, heavy ? 0.24 : 0.15),
          overRoad(heavy ? 0x251f5a : 0x2f2965, heavy ? 0.40 : 0.50), 0, 0.004, jz, flat));
        ...
        if (heavy) {
          parts.push(part(new THREE.PlaneGeometry(K.TRACK_HALF_WIDTH * 2, 0.11),
            overRoad(0x676782, 1.05), 0, 0.004, jz + 0.18, flat));
        }
      }
      /**
      ...
      for (const lx of [-LANE / 2, LANE / 2]) {
        for (const s of [-1, 1]) {
          ...
          parts.push(part(new THREE.PlaneGeometry(0.11, TILE), overRoad(0x534391, 0.80), lx + s * 0.12, 0.005, 0, flat));
        }
