...
MR.HUD = (function () {
  ...
  function create(root, opts) {
    ...
    root.innerHTML = `
      ...
      <div class="panel hidden" id="endPanel"><div class="panelInner">
        ...
        <div id="tomorrow" class="empty">
          ...
          <!--
            THE HOOK. The finish is the one moment motivation is spent and
            renewable at once, and the box used to end on a city name. One
            line names the reason to come back: the record that nearly fell
            today, or the roads still unstamped, or the bronze that can go
            gold -- always the player's own next rung, never a generic cheer.
          -->
          <div class="tomHook num" id="tomorrowHook"></div>
        </div>
    ...
    api.milesFinish = function (finishTime) {
      ...
      // The near-miss band widened to 45 on the behavioral review: a
      // 2:00:10 and a 2:10:00 used to get the same eulogy, and the agony of
      // almost is the strongest come-back-tomorrow fuel there is. Under 15
      // seconds Miles counts them; under 45 he names what they are worth --
      // one clean leg -- which is also, exactly, the coaching.
      const line = over <= 0 ? 'THAT&rsquo;S THE ONE.'
        : over <= 15 ? (over <= 1.5 ? 'ONE SECOND. TOMORROW.'
          : Math.ceil(over) + ' SECONDS. TOMORROW.')
        : over <= 45 ? Math.ceil(over) + ' SECONDS IS ONE CLEAN LEG.'
        : 'THE ROAD WON TODAY.';
    ...
    function drawShareCard(d) {
      ...
      const tint = d.medal === 'gold' ? ACCENT : d.medal === 'bronze' ? BRONZE : CREAM;
      g.save();
    ...
    function tomorrowLine() {
      try {
        ...
        if (s && s.length) {
          // The city AND its wager. A name alone is a fact; a name with the
          // record under it is a reason to come back.
          const names = s.map(function (x) { return x.name; }).join(' → ');
          return s[0].rec ? names + ' · RECORD ' + Pace.clock(s[0].rec) : names;
        }
    ...
    /**
     * The hook line under tomorrow's city: the player's own next rung, in
     * priority order. A record that nearly fell today outranks everything
     * -- that ache is the strongest return fuel there is -- then the roads
     * not yet run, then the bronze that can go gold, and a full-gold map is
     * a thing to defend. Empty when there is nothing true to say, which is
     * the same rule every plate on this card follows.
     */
    function hookLine(t, st0) {
      const overWR = t - K.RECORD_SECONDS;
      if (overWR > 0 && overWR <= 30) {
        return Pace.clock(overWR) + ' OFF THE WORLD RECORD · IT CAN FALL TOMORROW';
      }
      const overCity = st0 && st0.rec && st0.rec > K.RECORD_SECONDS ? t - st0.rec : Infinity;
      if (overCity > 0 && overCity <= 30) {
        return Pace.clock(overCity) + ' OFF THE COURSE RECORD · FINISH THE JOB';
      }
      const pool = MR.Course && MR.Course.SETTINGS;
      if (lastSum && lastSum.cities && pool && pool.length) {
        let unrun = 0, bronze = 0, gold = 0;
        for (const s of pool) {
          const tier = stampTier(s, lastSum.cities[s.name]);
          if (tier === 'new') unrun++;
          else if (tier === 'bronze') bronze++;
          else if (tier === 'gold') gold++;
        }
        if (unrun > 0) return unrun + ' ROAD' + (unrun === 1 ? '' : 'S') + ' STILL UNSTAMPED';
        if (bronze > 0) return bronze + ' BRONZE STAMP' + (bronze === 1 ? '' : 'S') + ' CAN GO GOLD';
        if (gold === pool.length) return 'EVERY STAMP IS GOLD · DEFEND THE MAP';
      }
      return '';
    }
    ...
    api.showEnd = function (p, rec, hits, guards) {
      ...
      if (st0) {
        ...
        n.stampCtx.classList.toggle('near',
          !medal && st0.rec && st0.rec > K.RECORD_SECONDS && t - st0.rec > 0 && t - st0.rec <= 30);
      ...
      n.tomorrowHook.textContent = tom ? hookLine(t, st0) : '';
