...
const three = read('vendor/three.min.js')
  // r160 UMD prints a deprecation warning on load; silence it for a clean console.
  .replace(/^console\.warn\((['"])Scripts[\s\S]*?\);/, '');
...
console.log(`built index.html  ${kb} KB  (${MODULES.length} modules)`);
